...
class FlatParamHandle:
    ...
    def _get_shard_metadata(
        ...
        for i, ((param_start, param_end), param_info) in enumerate(
            ...
            # needs refactor then.
            if (
                start > param_end or end < param_start
            ):
                continue
        ...
        if len(shard_param_indices_range) == 0:
            ...
            # Strictly less than results when there exists alignment padding
            # in the range
            assert (
    ...
    def _get_flat_param_offsets(self) -> List[Tuple[int, int]]:
        ...
        NOTE: This includes padding for alignment.
        """
    ...
    def _get_unflat_views(
        ...
        # TODO: Using `List[Tensor]` to avoid fixing all downstream type checks
        """
    ...
    def _use_sharded_grad_views(self) -> None:
        ...
        for i, (param, param_info) in enumerate(zip(flat_param._params, flat_param._param_infos)):
            if param_info is None:
                continue
            in_sharded_flat_param = (
                i >= start
