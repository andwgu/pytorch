...
class TestWrapUtils(TestCase):
    def test_validate_frozen_params(self):
        ...
        _validate_frozen_params(model, modules_to_wrap, set())
        ...
        with self.assertRaisesRegex(
            ValueError,
            "layers.0.attn has both parameters with requires_grad=True and "
            "False. FSDP does not support wrapping such modules.\n"
            ...
            r"\['layers.0.attn.q_proj.weight', 'layers.0.attn.k_proj.weight', 'layers.0.attn.v_proj.weight', 'layers.0.attn.o_proj.weight'\]"
        ):
            _validate_frozen_params(model, modules_to_wrap, set())
        ...
        _validate_frozen_params(model, modules_to_wrap, ignored_params)


instantiate_parametrized_tests(TestFSDPWrap)
instantiate_parametrized_tests(TestAutoWrap)
instantiate_parametrized_tests(TestWrapUtils)
