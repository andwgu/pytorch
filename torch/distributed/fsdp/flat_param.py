...
class FlatParamHandle:
    ...
    def _use_fp8_shard(self):
        flat_param = self.flat_param
        _p_assert(
            flat_param._mp_shard.dtype == torch.float8_e4m3fn, f"{flat_param.dtype}"
        )
        _alloc_storage(
            flat_param._mp_shard, flat_param._local_shard.size()  # type: ignore[attr-defined]
        )
    ...
    def _use_unsharded_views_fp8(self, as_params: bool) -> None:
        ...
        _p_assert(
            self._unused_leaf is not None,
            f"Unused leaf should have been set during init!",
        )
        if self._training_state == HandleTrainingState.FORWARD:
            _p_assert(torch.is_grad_enabled(), f"Requires grad to be enabled")
        views_float8 = SplitAndViewAsFloat8.apply(
            self._unused_leaf, self, self._fp8_post_backward_hook
        )
        # TODO: Deduplicate.
