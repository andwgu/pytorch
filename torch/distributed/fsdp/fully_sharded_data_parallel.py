...
class FullyShardedDataParallel(nn.Module):
    ...
    def _register_param_handle_from_params(
        ...
        print(f"[Rank {self.rank}] registered handle for {handle.flat_param._get_debug_name()}")
    ...
    def _register_pre_backward_hooks(self, outputs: Any, params: List[FlatParameter]) -> Any:
        ...
        # Reset before each backward pass
        self._params_to_pre_backward_hook_has_run[tuple(params)] = False
        ...
        def _pre_backward_hook(params: List[FlatParameter], *unused: Any) -> None:
            ...
            p_assert(
                tuple(params) in self._params_to_pre_backward_hook_has_run,
                f"Missing key in pre-backward for {params}",
            )  # TODO (awgu): remove
            if self._params_to_pre_backward_hook_has_run[tuple(params)]:
                return
