    ...
    *unused: Any,
    ...
    with torch.autograd.profiler.record_function(
        ...
        with state._device_handle.stream(state._post_backward_stream):
            ...
            autograd_computed_grad = flat_param.grad.data if not handle._use_fp8 else handle._autograd_computed_grad
            if (
...
@no_type_check
def _reduce_grad(state: _FSDPState, handle: FlatParamHandle, unsharded_grad: torch.Tensor) -> None:
    """
...
@no_type_check
def _reduce_grad_no_shard(state: _FSDPState, handle: FlatParamHandle, grad: torch.Tensor) -> None:
    """
