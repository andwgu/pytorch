...
def _get_fully_sharded_module_to_states(
    ...
    if not use_exec_order:
        # TODO (awgu): Gate the behavior to factor a shared parameter up to an
        # LCA module for now. For parent-child shared parameters, the existing
        # implementation already assigns to the parent.
        for shared_param_info, lca_module in shared_param_info_to_lca.items():
            if (
                lca_module is not shared_param_info.module1
                and lca_module is not shared_param_info.module2
            ):
                raise RuntimeError(
                    "Only parent-child shared parameters are supported. FSDP "
                    "found a shared parameter between the two modules:\n"
                    f"{shared_param_info.module1}\n{shared_param_info.module2}"
                )
